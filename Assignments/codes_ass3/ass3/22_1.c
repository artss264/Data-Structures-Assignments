...
/*
void minheapdelany(long long int element)
{
    long long int temp,tempindex,index,left,right,minim;
    for(i=1;i<=minheapend;i++)
    {
        if(minheap[i]==element)
            tempindex=i;
    }
    minheap[tempindex]=minheap[minheapend];
    minheapend--;
    if(minheap[tempindex]<minheap[tempindex/2])
    {
        while(tempindex/2>=1)
        {
            if(minheap[tempindex]<minheap[tempindex/2])
        ...
        else
        {
            while(2*tempindex<=minheapend)
            {
                ...
                if(right<=minheapend)
                {
                    if(minheap[right]<minim)
                    {
                        minim=minheap[right];
                        index=right;
                    }
                }
                if(minheap[tempindex]>minheap[index])
                {
                    temp=minheap[tempindex];
